...
public class ObjectManipulator : MonoBehaviour
{
    ...
    void GrabHoverObject(GameObject grbObj, Vector3 controllerPos, Quaternion controllerRot)
    {
        ...
        rotationOffset = 0.0f;
        if (grabObject.GetComponent<GrabObjects>())
        {
            grabObject.GetComponent<GrabObjects>().Grab(controller);
            grabObject.GetComponent<GrabObjects>().grabbedRotation = grabObject.transform.rotation;
            AssignInstructions(grabObject.GetComponent<GrabObjects>());
        }
    ...
    void ReleaseObject()
    {
        if (grabObject.GetComponent<GrabObjects>())
        {
            if (grabObject.GetComponent<GrabObjects>().objectManipulationType == GrabObjects.ManipulationType.Menu)
            {
            ...
            grabObject.GetComponent<GrabObjects>().Release();
        }
        grabObject = null;
    }
    ...
    void FindHoverObject(Vector3 controllerPos, Quaternion controllerRot)
    {
        ...
        if (objectInfo && objectInstructionsLabel)
        {
            ...
            if (hoverObject && !grabObject)
            {
                ...
                if (hoverObject.GetComponent<GrabObjects>())
                {
                    AssignInstructions(hoverObject.GetComponent<GrabObjects>());
                }
            }
            else if (grabObject)
            {
                ...
                if (grabObject.GetComponent<GrabObjects>()) showLaser = grabObject.GetComponent<GrabObjects>().showLaserWhileGrabbed;
            }
        }
        ...
        if (laser)
        {
            ...
            if (grabObject && grabObject.GetComponent<GrabObjects>())
            {
                grabObject.GetComponent<GrabObjects>().CursorPos(cursorPosition);
            }
        }
    }
    ...
    void ManipulateObject(GameObject obj, Vector3 controllerPos, Quaternion controllerRot)
    {
        ...
        if (obj.GetComponent<GrabObjects>())
        {
            useDefaultManipulation = false;
            switch (obj.GetComponent<GrabObjects>().objectManipulationType)
            {
                case GrabObjects.ManipulationType.Default:
                    useDefaultManipulation = true;
                    break;
                case GrabObjects.ManipulationType.ForcedHand:
                    ...
                    break;
                case GrabObjects.ManipulationType.DollyHand:
                    ...
                    obj.transform.rotation = Quaternion.Lerp(obj.transform.rotation, controllerRot, grabTime);
                    break;
                case GrabObjects.ManipulationType.DollyAttached:
                    ...
                    break;
                case GrabObjects.ManipulationType.HorizontalScaled:
                    obj.transform.position = controllerPos + controllerRot * localGrabOffset;
                    ...
                    Vector3 newFwd = obj.GetComponent<GrabObjects>().grabbedRotation * Vector3.forward;
                    ...
                    break;
                case GrabObjects.ManipulationType.VerticalScaled:
                    obj.transform.position = controllerPos + controllerRot * localGrabOffset;
                    if (!OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger, controller))
                    {
                        obj.transform.localScale = ClampScale(obj.transform.localScale, thumbstick);
                    ...
                    Vector3 newUp = obj.GetComponent<GrabObjects>().grabbedRotation * Vector3.up;
                    ...
                    break;
                case GrabObjects.ManipulationType.Menu:
    ...
    void AssignInstructions(GrabObjects targetObject)
    {
        if (objectNameLabel)
        {
